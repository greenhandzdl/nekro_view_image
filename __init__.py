...
class NekroViewImageConfig(ConfigBase):
    ...
    is_content_generated_by_bot: bool = Field(
        default=True,
        title="Is Content Generated by Bot",
        description="指示发送给模型的提示词是否由 AI 生成。（False则采用自定义的提示词）",
    )
    content: str = Field(
...
async def describe_image(_ctx: AgentCtx, image_data: str, prompt: str) -> str:
    """
    ...
    prompt: str
        提示词内容，会被添加到图片标签之前发送给模型，用于询问大模型关于图片的信息。
        建议：除了用户可能需要信息外，你可以加上整个图片信息的询问。
        比如，"Describe the image in detail."。
    ...
    # 3. 构造提示词（包含图片标签）
    ask_prompt = (prompt if config.is_content_generated_by_bot else config.content)
    prompt: str = f'{ask_prompt} <img src="{image_data}" />'
